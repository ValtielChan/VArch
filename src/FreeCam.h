...
//namespace CameraManipulators {

	class FreeCam : public CameraManipulator
	{
	public:
		FreeCam(Camera* camera) : CameraManipulator(camera) {}
		~FreeCam();
	...
	void FreeCam::processMove(CameraMovement direction, float deltaTime)
	{
		...
		if (direction == CameraMovement::FORWARD)
			m_camera->transform.translate(m_camera->m_front * velocity);
		if (direction == CameraMovement::BACKWARD)
			m_camera->transform.translate(-m_camera->m_front * velocity);
		if (direction == CameraMovement::LEFTWARD)
			m_camera->transform.translate(-m_camera->m_right * velocity);
